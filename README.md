...
This fork is specifically made for use on **AMD hardware using ROCm and MIGraphX**. It is made due to Migrapx requiring static input size.

* **`static_padding` Engine Toggle:** Allows initializing the engine with forced static shape token tensors (`MAX_TOKENS = 510`), keeping execution graphs stable.
* **Nondestructive Token Auto-Chunking:** Under the hood, phoneme tokens that exceed the maximum constraint are automatically split into ideal sub-sequences and generated sequentially without any text or information loss.
